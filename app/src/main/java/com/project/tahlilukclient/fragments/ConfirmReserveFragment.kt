...
private lateinit var confirmReserveBinding: FragmentConfirmReserveBinding
    ...
    override fun onCreateView(
        ...
        currentLocation = LatLng(bundle.getDouble(Constants.SELECTED_ADDRESS_LATITUDE),bundle.getDouble(Constants.SELECTED_ADDRESS_LONGITUDE))
        lab = bundle.getSerializable(Constants.SELECTED_LAB) as Lab
        (bundle.getSerializable(Constants.SELECTED_ANALYTICS) as ArrayList<Analytics>).also { selectedAnalyticsList = it }
    ...
    private fun setAnalyticsList(){
        if (selectedAnalyticsList.isNotEmpty()){
            ...
            confirmReserveBinding.rvCartListItems.layoutManager = LinearLayoutManager(requireContext())
    ...
    private fun setReceiptSection(){
        ...
        (mSubTotal.toString() + getString(R.string.egyptian_currency)).also { confirmReserveBinding.tvCheckoutSubTotal.text = it }
        ("50"+getString(R.string.egyptian_currency)).also { confirmReserveBinding.tvCheckoutShippingCharge.text = it }

        mTotalAmount = mSubTotal+mVisitingFee
        confirmReserveBinding.tvCheckoutTotalAmount.text = mTotalAmount.toString() + getString(R.string.egyptian_currency)
    }


    private fun setListeners(){
        confirmReserveBinding.btnSaveAnalytics.setOnClickListener {
            SupportFunctions.loading(true,confirmReserveBinding.btnSaveAnalytics, confirmReserveBinding.progressBar)
            val reserve = Reserve()
